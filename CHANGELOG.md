...
### Fixed
- **Mid-suite test crashes on cross toolchains (W-357)**: three
  ...
  - `Lean.Elab.Term.Context` was built with the 4.25/4.26 layout
    ...
    with `isMetaSection` / `checkDeprecated` scalars arriving only in
    **4.29** (8 + 9 scalars on 4.27–4.28, 8 + 11 on 4.29+; verified
    against the `TermElabM.lean` source of every release from v4.25.2
    through the 4.34-nightly commit). On 4.27–4.30 the 11 scalar bytes
    were written over the `fixedTermElabs` object pointer, so the first
    incref of that field SIGSEGV'd every `run_tactic` call
    (`test_meta_repl` on 4.28.0 / 4.30.0). `default_term_context` now
    has one branch per layout: < 4.27 (7 + 11), 4.27–4.28 (8 + 9),
    4.29+ (8 + 11).
