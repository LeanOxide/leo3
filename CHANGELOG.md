...
- **`meta::keepalive`** (Linux): stopgap for the cross-module-set SIGSEGV that
  ...
  Lean file region (`.olean` and its companion `.olean.server` /
  `.olean.private` / `.ir` mappings) is `mmap`ed at a deterministic base
