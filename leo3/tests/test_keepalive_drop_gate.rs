...
fn drop_gate_frees_file_backed_and_leaks_heap_backed() {
    let result: LeanResult<()> = leo3::test_with_lean(|lean| {
        ...
        let before_a = snapshot_lean_vmras();
        let a = import_modules(lean, &["Lean"], 0)?;
        let after_a = snapshot_lean_vmras();
        let added_a = diff_added_vmras(&before_a, &after_a).len() as u64;
        let count_a = environment_region_count(a.as_ptr() as *const std::ffi::c_void);
        assert!(
        ...
        let before_b = snapshot_lean_vmras();
        let b = import_modules(lean, &["Lean"], 0)?;
        let after_b = snapshot_lean_vmras();
        let added_b = diff_added_vmras(&before_b, &after_b).len() as u64;
        let count_b = environment_region_count(b.as_ptr() as *const std::ffi::c_void);
        assert!(
